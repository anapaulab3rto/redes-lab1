...
original_bits = "101100111111100101"
snr=-3

SNR_VALUES = np.arange(0, -102, -2)
error_counts = []
...
for snr in SNR_VALUES:
    ...
    # Conta erros
    errors = sum(1 for a, b in zip(original_bits, decoded) if a != b)
    error_counts.append(errors)

    print(f"SNR = {snr} dB → Erros = {errors}")


for snr, err in zip(SNR_VALUES, error_counts):
    if err > 0:
        print("\n(a) Primeiros erros aparecem em SNR =", snr, "dB")
        break

for snr, err in zip(SNR_VALUES, error_counts):
    if err == len(original_bits):
        print("(b) Todos os bits comprometidos em SNR =", snr, "dB")
        break

plt.figure(figsize=(10,5))
plt.plot(SNR_VALUES, error_counts, marker='o')
plt.xlabel("SNR (dB)")
plt.ylabel("Número de Erros")
plt.title("Impacto do Ruído na Decodificação NRZ")
plt.grid(True)
plt.show()